...
uniform sampler2D uGbufferEmission;
...
void main( void ) {
	...
	vec4 gCol2 = texture( uGbufferEmission, vUv );
	...
	outColor *= max( 0.0, 1.0 - texture( uSSAOTexture, vUv ).x * 4.0 * ( 1.0 - length(gCol2.xyz) ) );
	outColor += texture( uLightShaftTexture, vUv );
